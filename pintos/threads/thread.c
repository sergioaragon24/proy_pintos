...
void int_list_wait(int64_t ticks){
  ...
  thread_actual->sleep_endtick = ticks;
  
  /* timer_ticks() +
  Donde TIEMPO_DORMIDO es el atributo de la estructura thread que usted
	  definió como paso inicial y es sleep_endtick*/
...
void rem_thread_sleep(int64_t ticks){
	...
	while(iter != list_end(&sleep_list) ){
		...
		if(ticks >= thread_lista_espera->sleep_endtick){
